...
export default function ScanPage() {
  ...
  // --- FUNGSI YANG DIPERBARUI UNTUK MENGIRIM LOG ---
  const handleConfirmStatus = async () => {
    ...
    try {
      ...
      // 2. Update status di tabel mesin_incinerator
      const updateTask = supabase
        .from('mesin_incinerator')
        .update({ status: selectedStatus })
        .eq('id', scannedId);

      // 3. Masukkan riwayat ke tabel aktivitas_log
      const logTask = supabase
        .from('aktivitas_log')
        .insert([{
          mesin_id: scannedId,
          nama_mesin: machineName,
          operator_id: user?.id,
          operator_name: user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'Operator', // Ambil nama dari profil Google
          aksi: 'Perubahan Status',
          keterangan: selectedStatus === 'on' ? 'Mesin ON' : 'Mesin OFF'
        }]);
